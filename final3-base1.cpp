...
class Compression
{
  public:
    ...
    Compression(int n){
      Dsize =0;
      primes[]={0x30D7F, 0x61A9F, 0xF422F, 0x989693, 0x5F5E107, 0x3B9AC98B};
      lookup[]={3,4,4,4,5,6};
      arraysize = primes[n];
    ...
    void LZWComp2(char str[], char str2[]){	
      LZWComp(str,str2,m);
		...
		void LZWDecomp(char str[], char str2[],int n){		//read(starting from second charecter , as first we already read) a character k in blocks of n+3; // eg if n=0 pick up blocks of 3 from text
			FILE * fp;
			FILE * fp2;
			if((fp=fopen(str,"r")) == NULL){
